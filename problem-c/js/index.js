...
function huskiesLost(game) {
  return game.home == 'UW' ? game.opponent_score > game.home_score : 
...
let diffOpponents = gamesLost.map(extractOpponent);
console.log(diffOpponents);
...
gamesLost.forEach(game => console.log(game.opponent + " at " + game.home + ", " + game.opponent_score + " to " + game.home_score));
...
console.log(huskyGames2016.filter(games => games.fumbles > 0).length);
...
function mostYardsPassing(game1, game2) {
  if (game1.passing_yards == null) {
    return game2;
  } else if(game2.passing_yards == null) {
    return game1;
  } else {
    return game2.passing_yards - game1.passing_yards > 0 ? game2 : game1;
  }
}
...
function makeCombinedFilter(filter1, filter2) {
  return function(game) { return filter1(game) && filter2(game) };
}
