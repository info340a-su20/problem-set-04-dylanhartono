...
*/
function compoundInterest(p, i, t) {
...
for(let i = 2; i <= 14; i++) {
   for(let j = 0; j < suit.length; j++) {
    deck[deck.length] = {suit : suit[j], rank : i};
  }
}
console.log(deck);
...
function containsQueenOfHearts(pokerHand) {
  return pokerHand.filter(card => (card.rank == 12 && card.suit == 'hearts')).length == 1;
}
...
function getHighCard(pokerHand) {
  return pokerHand.sort((a, b) => a.rank - b.rank).reverse()[0];
}
...
function isFlush(pokerHand) {
  return pokerHand.every((val, i, arr) => val.suit === arr[0].suit);
}
